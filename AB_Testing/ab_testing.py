...
# A sample is a subset of a population that is assumed to well represent the characteristics of that population.
# That is, it is representative of its population.

# yapacak olduğumuz şey popülasyon ve örneklem arasındaki ilişkinin veri dünyasına ne şekilde yansıdığını görmeye çalışmaktır.

population = np.random.randint(0, 80, 10000)  # 0-80 arası 10000 random sayı oluşturuyoruz.
population.mean()

np.random.seed(115)
# Seed function is used to save the state of a random function, so that
# it can generate same random numbers on multiple executions of the code
# on the same machine or on different machines (for a specific seed value).

sample = np.random.choice(a=population, size=100)  # population içerisinden 100 elemanlı örneklem seçildi.
sample.mean()
...
# What is meant here is that when the number of samples increases,
# the mean of this sample distribution will get closer to the population mean.
# Therefore, when the data increases or the sample increases, there is a convergence.

# Since each of the selected samples in the population expresses the average of the population
# at a different observation point, we can make a stronger inference when we select
# more than one sample and take the average of them.
...
# How to Interpret :
# - If the distribution of the variable we have is skewed, that is,
#   it contains outliers, then the median should be used instead of mean value
#   to represent this variable. Otherwise, a misleading result may be obtained.

# To find out if the variable has an outlier :
# - The mean and median values are compared. If these values are very close to each other, then it will not make
#   any difference whether the statistic we will use to represent the relevant variable is the median or the mean.
#   However, if there is a significant difference between the two values, then it would be more appropriate to
#   give the mean, median and standard deviation information.
